...
在引擎中，大量存在着通用游戏物体 `object`，它们通过放置在房间之后的 `Creation Code` 来实现差异化功能。具体使用方法为：
...
3.  `Creation Code`中，存在着许多通用代码，记住它们将能帮助你更快更好地使用引擎，归类如下：
...
用来绘制 boss 血条。这仅仅是一个如何使用 `drawLife` 函数的范例
